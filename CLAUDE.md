...
- **Go KV 存储系统学习 Demo**。v1 是单机 LSM Tree 存储引擎(WAL → MemTable → SSTable → Compaction),v2 启动分布式演进。
- **v1 主流程已实现**:支持 `Open` / `Put` / `Get` / `Delete` / `Flush` / `Compact` / `Close`,包含 WAL、MemTable、SSTable、恢复和手动 Compaction。
- 当前代码仍是**单机库**:暂时没有 `cmd/` 目录,没有网络层。v2 路线会按阶段新增 gRPC、Raft 和 shard。`example/simple_example.go` 是可运行 API 示例。
...
go test ./internal/engine/record/...                    # 跑单个包
...
- 根目录 `package kv` 是**对外单机 API**(`db.go` / `options.go` / `errors.go`)。使用者这样导入:
...
- v2 分布式目录按阶段创建,不要提前创建空壳。目标结构见 `docs/agent/layout.md`。
...
3. **不要一次性生成完整引擎**。每个模块都要先讲:模块职责、与 LSM Tree 的关系、数据结构、伪代码、测试点,**等用户确认后再写代码**(详见 `docs/agent/workflow.md`)。
   v2 分布式阶段还要先讲清楚网络接口、复制语义、路由规则和测试点。
4. **测试放对位置**:
...
- `docs/distributed.md` — **v2 分布式演进设计**(gRPC、Raft、一致性哈希、shard group)。
...
1. 读 `README.md` → `AGENTS.md` → `docs/design.md`。如果涉及分布式,继续读 `docs/distributed.md`。
